defmodule OatFirstWeb.CoreComponents do
  @moduledoc """
  ...
  The foundation for styling is the [Oat CSS framework](https://oat.ink), which provides
  a utility-based class system and UI components for layout, forms,
  buttons, tables, and more.
  ...
  use Gettext, backend: OatFirstWeb.Gettext
  ...
  def button(%{rest: rest} = assigns) do
    if rest[:href] || rest[:navigate] || rest[:patch] do
      ~H"""
      <.link class={["button", @class]} data-variant={@variant} {@rest}>
        {render_slot(@inner_block)}
      </.link>
      """
    else
      ~H"""
      <button class={@class} data-variant={@variant} {@rest}>
        {render_slot(@inner_block)}
      </button>
      """
    end
  ...
  def input(%{type: "checkbox"} = assigns) do
    ...
    ~H"""
    <div data-field={@errors != [] && "error"} class="mb-2">
      <label>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(OatFirstWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(OatFirstWeb.Gettext, "errors", msg, opts)
    end
  end
